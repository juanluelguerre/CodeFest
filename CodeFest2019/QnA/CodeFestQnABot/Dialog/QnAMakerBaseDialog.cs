...
namespace Microsoft.BotBuilderSamples.Dialog
{
    ...
    public class QnAMakerBaseDialog : ComponentDialog
    {
		...
		public const string DefaultNoAnswer = "Disculpa, no tengo respuesta para eso, tendrías que enseñarme primero.";

		// Card parameters
		public const string DefaultCardTitle = "Quieres decir:";
        public const string DefaultCardNoMatchText = "Ninguna de las anteriores.";
        public const string DefaultCardNoMatchResponse = "Gracias por tu feedback.";
		...
		private readonly IHttpClient _httpExternaClient;
        ...
        public QnAMakerBaseDialog(IBotServices services, IHttpClient httpExternalClient) : base(nameof(QnAMakerBaseDialog))
        {
			...
			_httpExternaClient = httpExternalClient;
        ...
        private async Task<DialogTurnResult> DisplayQnAResult(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            if (stepContext.Result is List<QueryResult> response && response.Count > 0)
            {
				...
				if (response.First().Questions.All(q => q.Contains("Twitter", StringComparison.InvariantCultureIgnoreCase))
					&& response.First().Metadata.First().Name.Equals("CodeFest", StringComparison.InvariantCultureIgnoreCase)
					&& response.First().Answer.Equals("Un momento, que lo consulto", StringComparison.InvariantCultureIgnoreCase))
				{
					await stepContext.Context.SendActivityAsync("Consultando...", cancellationToken: cancellationToken).ConfigureAwait(false);


					var resultJson = await _httpExternaClient.GetStringAsync("https://codefesttwitterapi.azurewebsites.net/api/twitter");
					var result = JsonConvert.DeserializeObject<List<Twitter>>(resultJson);

					var numOfTwitters = result.LongCount().ToString("0,0", CultureInfo.GetCultureInfo("ES-es"));
					await stepContext.Context.SendActivityAsync($"{numOfTwitters} Twitters !", cancellationToken: cancellationToken).ConfigureAwait(false);
				}
			}
            else
            {
